...

{
  ...
  "$scheduleId": {
    ...
    ".write": "
      auth != null &&
      (
        // 1. Create: conductor must be assigned to bus
        (!data.exists() && newData.child('conductorId').val() == auth.uid &&
         root.child('busAssignments').child(newData.child('busId').val()).child('conductorId').val() == auth.uid) ||

        // 2. Update/Delete: must be conductor AND schedule must be in future
        (data.exists() && data.child('conductorId').val() == auth.uid &&
         root.child('busAssignments').child(data.child('busId').val()).child('conductorId').val() == auth.uid &&
         data.child('startTime').val() > now)
      )",
    ".validate": "newData.hasChildren(['busId', 'conductorId', 'startTime', 'endTime', 'date', 'createdAt', 'direction']) &&
                  ...
                  newData.child('direction').isString() &&
                  (newData.child('direction').val() == 'going' || newData.child('direction').val() == 'returning')"
  }
},
    "requests": {
      ".indexOn": ["status", "riderId", "acceptedBy", "busId"],
...

{
  "rules": {
    ...
    "requests": {
      ".indexOn": ["status", "riderId", "acceptedBy", "busId"],
      ".read": "auth != null && (query.orderByChild == 'status' && query.equalTo == 'Pending' && root.child('users').child(auth.uid).child('role').val() == 'Conductor') ||
      (query.orderByChild == 'status' && query.equalTo == 'Accepted' && root.child('users').child(auth.uid).child('role').val() == 'Owner') ||
        (query.orderByChild == 'status' && query.equalTo == 'Accepted' && root.child('users').child(auth.uid).child('role').val() == 'Conductor') ||
      (query.orderByChild == 'riderId' && query.equalTo == auth.uid) || (query.orderByChild == 'acceptedBy' && query.equalTo == auth.uid)",
      "$id": {
        ".read": "auth != null && (data.child('riderId').val() == auth.uid || data.child('acceptedBy').val() == auth.uid || (root.child('users').child(auth.uid).child('role').val() == 'Conductor' && data.child('status').val() == 'Pending'))",
        ".write": "auth != null && (
          (newData.child('riderId').val() == auth.uid && (
            !data.exists() ||
            (data.child('status').val() == 'Pending' && data.child('riderId').val() == auth.uid)
          )) ||
          (root.child('users').child(auth.uid).child('role').val() == 'Conductor' &&
        ...
        ".validate": "
          (
            newData.child('status').val() == 'Cancelled' &&
            data.child('status').val() == 'Pending' &&
            newData.child('riderId').val() == data.child('riderId').val()
          ) || (
            newData.hasChildren(['id', 'riderId', 'pickup', 'destination', 'pickupLatLng', 'destinationLatLng', 'seats', 'fare', 'status', 'createdAt']) &&
            (newData.child('status').val() == 'Pending' || newData.hasChild('busId')) &&
            (!newData.hasChild('busId') || newData.child('busId').isString()) &&
            newData.child('pickup').isString() &&
            newData.child('destination').isString() &&
            ...
            newData.child('destinationLatLng').hasChildren(['lat', 'lng']) &&
            newData.child('seats').isNumber() &&
            newData.child('seats').val() > 0 &&
            newData.child('fare').isNumber() &&
            newData.child('fare').val() >= 0 &&
            newData.child('status').isString() &&
            (newData.child('status').val() == 'Pending' || newData.child('status').val() == 'Accepted' || newData.child('status').val() == 'Cancelled') &&
            (!newData.child('otp').exists() || newData.child('otp').isString()) &&
            (!newData.child('conductorId').exists() || newData.child('conductorId').isString()) &&
            (!newData.child('acceptedBy').exists() || newData.child('acceptedBy').isString()) &&
            (!newData.child('scheduleId').exists() || newData.child('scheduleId').isString()) &&
            (!newData.child('acceptedAt').exists() || newData.child('acceptedAt').isNumber())
          )"
      }
    },
    "conductorLocations": {
...
================20 Nov 2025 ========================================
{
  "rules": {
    ...
    "schedules": {
      ...
      "$scheduleId": {
        ".read": "auth != null",
        ".write": "auth != null && ((!data.exists() && newData.child('conductorId').val() == auth.uid && root.child('busAssignments').child(newData.child('busId').val()).child('conductorId').val() == auth.uid) || (data.exists() && data.child('conductorId').val() == auth.uid && root.child('busAssignments').child(data.child('busId').val()).child('conductorId').val() == auth.uid && data.child('startTime').val() > now))",
        ".validate": "newData.hasChildren(['busId', 'conductorId', 'startTime', 'endTime', 'date', 'createdAt', 'direction']) && newData.child('startTime').isNumber() && newData.child('endTime').isNumber() && newData.child('date').isString() && newData.child('createdAt').isNumber() && newData.child('direction').isString() && (newData.child('direction').val() == 'going' || newData.child('direction').val() == 'returning')"
      }
    },
    "requests": {
    ...
    "conductorLocations": {
      ...
      "$conductorId": {
        ".read": "auth != null && (auth.uid == $conductorId || root.child('users').child($conductorId).child('ownerId').val() == auth.uid)",
        ".write": "auth != null && auth.uid == $conductorId",
        ".validate": "newData.hasChildren(['lat', 'lng', 'timestamp']) && newData.child('lat').isNumber() && newData.child('lng').isNumber() && newData.child('timestamp').isNumber()"
      }
    },
